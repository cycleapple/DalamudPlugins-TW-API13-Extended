...
| MouseLock | 已完成 | 1.0.0.5；完成首次啟動、四個設定分頁、狀態列、按鍵錄製、快捷列選擇器、視窗例外、匯入／備份、通知、聊天指令回饋及歷史變更記錄；指令 token、設定鍵、輸入綁定、IPC 固定回傳值與 Hook 邏輯未變更 |
| Mask of Kefka | 已完成 | 0.2.0.1；完成設定視窗、效能與 OBS 指引、無 UI 模式、渲染目標、輸出頻率、指令說明及 manifest；`on`／`off`／`enable`／`disable`／`ui` 指令 token、設定鍵、DX11 資源共享與渲染目標索引邏輯未變更 |
