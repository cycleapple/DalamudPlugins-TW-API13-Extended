...
| AutoRetainer | 第三輪完成 | 4.5.1.25；新增完整雇員委託規劃器，涵蓋計畫載入／命名／儲存／覆寫／刪除、完成行為、委託搜尋、等級範圍、不可用委託、快速探險與共用計畫提示；enum 僅新增顯示映射。遊戲文字比對、Addon、Regex、IPC、指令、序列化與自動化流程未變更 |
...
| AutoDuty | 第三輪完成 | 0.0.0.4；新增主操作頁路徑狀態、缺檔說明、依賴缺失、執行前驗證、練等模式、親信戰友、解除限制、職業限制與等待狀態 zh-TW；尚待路徑編輯器、路徑頁次級按鈕、資訊頁與開發者設定。指令 token、IPC、Addon、列舉／序列化值、路徑格式、導航判斷與自動化未變更 |
