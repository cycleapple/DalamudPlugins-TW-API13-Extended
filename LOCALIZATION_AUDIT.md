...
| Artisan | 已完成 | 4.0.4.18；補齊耐久製作錯誤、消耗品預設狀態、模擬狀態／結果、技能不可用原因與巨集能力值提示；另修正素材清單即時更新、半成品抵扣統計、過期推薦工作逾時及製作結束 Null 例外；保留與巨集解析共用的英文技能識別名稱，DebugTab、內部 ID、命令、IPC、序列化與遊戲資料未變更 |
...
| Ice's Cosmic Exploration | 已完成 | 0.0.73.49；完成 manifest、主側欄、模式選擇、停止條件、任務／職業優先序、信用點購物、宇宙轉盤、安全、採集設定、浮層、Waypoint、任務表／詳情／統計、必要插件與紀錄等一般使用者介面；移除可停用宇宙探索確認視窗安全比對的介面選項；任務／技能／物品資料、IPC、ID與序列化格式未變更 |
...
| Chat 2 | 已完成 | 1.32.0.2；426 個 zh-Hant 鍵齊全，補齊資料庫瀏覽器／進階維護／搜尋／字體重設 UI，並重建內含登入、標籤、輸入、傳送、錯誤與驗證訊息的繁中 Web 前端；開發者 Debugger／SeStringDebugger、內部識別字與品牌名稱依翻譯邊界保留原文 |
