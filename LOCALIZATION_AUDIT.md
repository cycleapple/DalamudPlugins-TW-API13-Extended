...
| Lifestream | 已完成 | 2.5.3.8；完成核心設定、房屋路徑、Wotsit、浮層、通訊錄、Custom Alias 與 PaissaDB 介面；另修正台服八個世界的正確 ID、中文／英文名稱解析、陸行鳥資料中心分組、Materia 誤列及巴哈姆特同名舊資料造成的選單遺漏；指令 token、序列化與 PaissaDB 協定未變更 |
...
| GatherBuddy Reborn | 已完成 | 7.3.5.5；使用者介面與台灣用語 zh-TW 完成。上游 `TimeInterval.cs` 的 GBK/CP936 編碼已轉為合法 UTF-8，並以 7 個分支測試驗證「天／小時／分鐘／永遠／從不／無效」；另依實際 `FishParameter` 判斷區域資料可用性，此為相容性修正，未變更遊戲文字比對、序列化、AutoHook IPC、導航或 DataId |
...
| AutoDuty | 已完成 | 0.0.0.7；完成主視窗頁籤、路徑編輯／管理、狀態、快速按鈕、指令說明、練等模式、排隊、修理、分解、精製、寶箱、大國防聯軍、九宮幻卡與住宅移動等一般使用者介面；另修正 RSR IPC 合約中的舊 `TargetOnly` 枚舉偏移，未改動路徑資料或戰鬥邏輯 |
