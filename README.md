...
| Wrath Combo | 1.0.1.21 | 整合連段 | 已發布；補齊功能清單與職業設定 zh-TW |
...
| Artisan | 4.0.4.15 | 生產輔助 | 已發布；補齊耐久製作錯誤、模擬結果、技能不可用原因與巨集能力值提示 |
...
| Lifestream | 2.5.3.5 | 傳送輔助 | 已發布；完成房屋路徑、跨服限制、Wotsit、浮層、通訊錄、Custom Alias 與 PaissaDB 介面 zh-TW |
...
| XIV 藏寶圖工具小幫手 | 0.1.9.3 | 繁中藏寶圖組隊協作與網頁同步 | 已發布；座標訊息可自訂聊天頻道指令（例如 `/p`、`/cwl1`） |
