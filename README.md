...
| AutoRetainer | 4.5.1.30 | 雇員相關功能 | 已發布；修正台服繁中「自由尋寶委託」無法自動選取，並保留既有 zh-TW 介面 |
...
| Artisan | 4.0.4.26 | 生產輔助 | 已發布；修正宇宙探索收藏品分級門檻，選擇最低品質時不再製作至滿品質 |
...
| Lifestream | 2.5.3.9 | 傳送輔助 | 已發布；新增 `/li ren` 前往傳喚鈴，Custom Alias 支援等待與自訂 Slash 指令；保留台服世界 ID 修正 |
| vnavmesh | 1.1.2.5 | 尋路與移動工具 | 已發布；月面 Cosmoliner 改依有效 layout 模型逐對建立連線，並補上 SS 與隧道間的安全雙向連線 |
| AutoDuty | 0.0.0.9 | 副本自動化框架 | 已發布 API13；新增可保存進度的副本排程器，可依序設定各副本的路徑與執行次數 |
| Pixel Perfect | 3.3.2.1 | 顯示角色碰撞圈 | 已發布；完成 zh-TW 介面翻譯 |
| LazyLoot | 5.3.2.5 | 戰利品擲骰輔助 | 已發布；完成 zh-TW 介面、指令說明與診斷訊息翻譯 |
| Questionable | 13.68.129.8 | 任務輔助 | 已發布；新增安全 Auto 接續模式與最多 5 次指數退避重試；使用者介面 zh-TW 完成 |
...
| Visland | 0.0.0.139 | 無人島採集、工坊排程與活動自動化 | 已發布；完成 zh-TW 翻譯，並支援匯入 OC Discord 英文工房排程 |
| Chat 2 | 1.32.0.3 | 高度自訂的遊戲聊天視窗替代方案 | 已發布；新增台灣繁中字形、CJK 標點及全形／半形符號範圍，並保留完整 zh-TW 介面 |
...
| Waymark Preset Plugin | 1.5.5.0 | 儲存、編輯與套用場地標記預設 | 已發布；API13；繁中介面待處理 |
| Honorific | 1.7.1.3 | 自訂角色稱號與名牌顯示 | 已發布；API13；繁中介面待處理 |
| NOTED | 1.6.0.0 | 依職業或副本保存遊戲內筆記 | 已發布；API13；繁中介面待處理 |
| Flash on Tell | 0.0.9.0 | 收到悄悄話時閃爍工作列圖示 | 已發布；API13；繁中介面待處理 |
| Custom Resolution | 1.0.1.0 | 自訂遊戲解析度與介面縮放 | 已發布；API13；繁中介面待處理 |
| Market Board | 1.11.0.0 | 改善市場布告板搜尋與顯示 | 已發布；API13；繁中介面待處理 |
| Peeping Tom | 1.7.13.1 | 顯示曾以你為目標的玩家 | 已發布；由維護分支回移至 API13；繁中介面待處理 |
| Eureka Linker | 1.5.9.5 | 自動整理並連結探索區域追蹤資訊 | 已發布；API13；繁中介面待處理 |
| Hunt Buddy | 1.2.2.0 | 追蹤狩獵通緝令與目標進度 | 已發布；API13；繁中介面待處理 |
| ZodiacBuddy | 1.13.0.2 | 追蹤古武製作階段與魂光進度 | 已發布；API13；繁中介面待處理 |
| EasyEyes | 1.3.1.1 | 過濾或停用指定視覺特效 | 已發布；API13；繁中介面待處理 |
| Fungah | 1.0.0.7 | 顯示仙人刺與部分場景的安全位置 | 已發布；API13；繁中介面待處理 |
| Visibility | 1.1.9.2 | 隱藏玩家、召喚獸、陸行鳥與寵物 | 已發布；API13；繁中介面待校訂 |
| Enemy List Debuffs | 1.13.0.0 | 在敵對列表顯示負面狀態 | 已發布；API13；繁中介面待處理 |
| Penny Pincher | 1.8.0.6 | 協助調整市場布告板上架價格 | 已發布；API13；繁中介面待處理 |
| Housemate | 1.0.3.4 | 顯示房屋物件資訊並輔助裝修 | 已發布；API13；繁中介面待處理 |
| MouseLock | 1.0.0.3 | 不需持續按住右鍵的滑鼠視角鎖定 | 已發布；移植至台服 API13；設定介面待翻譯；涉及滑鼠輸入與遊戲函式 Hook |
