...
| Artisan | `MeowZWR/Artisan` + `4liang0121/Artisan@6d10618` | `e4a0ddaf` | 2026-08-21 | `cycleapple/Artisan@348d3ab` 保留既有台服 API13 與 Raphael 生命週期修正；採用 fork 的宇宙配方精確定位概念，加入開啟節流、15 秒逾時、選取結果驗證與 IPC 失敗原因，並防止未選配方時讀取 Material Miracle 設定；排除共通巨集、暫時設定、製作預設與未具來源提交的 Raphael binary；API13 Release 建置 0 錯誤，發布 4.0.4.24；上游已封存 |
...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` + `4liang0121/Ices-Cosmic-Exploration@21a4935` | `5ecca37` | 2026-08-21 | 保留既有 ECommons、台服 API13、zh-TW 與 Artisan 同步修正；選擇性採用 fork 的逾時成果檢查、採集設定、任務繳交及任務預設概念，並加強有限節點、Critical 繳交範圍、設定保存與未知任務 ID 防護。製作預設 IPC 因暫時覆寫會殘留且缺少消耗品 ID 驗證而排除；Agenda、跨區移動、共通巨集及整批路線資料仍未納用；API13 Release 建置 0 錯誤，發布 0.0.73.51 |
