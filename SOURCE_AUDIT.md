...
| GatherBuddy Reborn | `AtmoOmen/GatherBuddyReborn` | `a34733dc` / tag `7.3.5.0` | 2025-12-02 | 位於安全日期範圍且 manifest 為 API13；`cycleapple/GatherBuddyReborn@45068565` 保留台服繁中 fallback、`DataId` 與 AutoHook IPC 容錯及時間文字編碼修正；比對 `ffxiv-datamining-tc` 後確認台服缺少 `46188`、`47422`、`47680` 等 The Promise of Tomorrow 魚類資料，改以 `FishParameter` 哨兵列判斷後才套用該批資料，並修正 Miounnefish 多段以小釣大鏈缺少基礎魚餌 `20615`；API13 建置 0 錯誤，發布 7.3.5.5 |
| Artisan | `MeowZWR/Artisan` | `e4a0ddaf` | 2025-12-13 | `cycleapple/Artisan@c8dac53` 保留 `BaseId`、熱鍵列空參考與台服相容修正；修正素材清單庫存失效、依配方依賴順序重算半成品抵扣，並阻止舊背景工作覆蓋最新清單；API13 建置 0 錯誤，發布 4.0.4.18；上游已封存 |
...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` | `a9f6d9292b` | 2026-08-02 | 保留 ECommons、`DataId` 與 `Player.JobId` 台服 API13 修正並完成一般使用者介面 zh-TW；依 `HoshinoCorp/ffxiv-datamining-tc` 的 `ENpcResident`／`Level` 加入領地 1237 各探索階段的研究威、修理、交換與好運道 NPC ID，並保留座標附近安全備援；合併 PR #1，修正任務內導航判斷，改善任務分類按鈕、轉盤圖示、必要插件倉庫與採集技能／技巧點翻譯；API13 建置 0 錯誤，發布 0.0.73.45 |
