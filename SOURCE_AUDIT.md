...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` | `4c2fc71` | 2026-08-20 | 保留 ECommons、`DataId` 與 `Player.JobId` 台服 API13 修正並完成一般使用者介面 zh-TW；將宇宙探索確認視窗安全比對改為不可停用，防止流程誤接非預期視窗；`cycleapple/ECommons@fdf444c` 補上任務分數 UI 載入中的索引、類型與空指標防護；雙職任務在 ICE 要求的 Artisan 製作完成後可忽略殘留 busy 狀態並轉回採集；API13 建置 0 錯誤，發布 0.0.73.49 |
