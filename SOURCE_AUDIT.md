...
| AutoRetainer | `PunishXIV/AutoRetainer` | `1e4e6b6` / tag `4.5.1.12` + `11e55d7` QuickExploration hunk | 2026-07-03 | `cycleapple/AutoRetainer@0747a7f` 保留繁中公會工坊／組件功能比對與完整 zh-TW 介面，回移上游 `tw` 分支目前使用的「自由尋寶委託」比對；使用 API13 Dalamud 組件及 .NET SDK 10.0.302 建置（40 個既有警告、0 錯誤），發布 4.5.1.30 |
...
| Artisan | `MeowZWR/Artisan` + `4liang0121/Artisan@6d10618` + `PunishXIV/Artisan@116e982` | `e4a0ddaf` | 2026-08-25 | `cycleapple/Artisan@68fc306` 保留台服 API13、Raphael 生命週期、宇宙配方選取、暫時 IPC 與附近玩家延遲修正；移植上游 WKS 評價表門檻換算，修正宇宙收藏品低／中／高品質全被設為滿品質。API13 Release 完整建置 0 錯誤，發布 4.0.4.26；上游已封存 |
...
| Price Insight | `cycleapple/ffxiv-priceinsight` (`api13-tw`) | `d41cd5c` | 2025-08-07 基底 | API13 編譯 0 警告、0 錯誤；繁中服染料在 aggregated API 拒絕時改用三路受限 v3 overview 相容查詢；404 改為逐世界復核、不完整 DC／區域不顯示聚合值；20 筆成交截斷時不計算平均價及速度，並支援 nullable 成交數量、互動查詢優先序、10 分鐘空資料快取與 5 分鐘失敗冷卻。以 126 種可搜尋染料實測：21 種由 v2 回傳，105 種 fallback 全部獲得 v3 200（94 種有資料、11 種空資料、0 錯誤），發布 2.11.3.5 |
...
| Visland | `awgil/ffxiv_visland` | `f7f40e1` / tag `v0.0.0.137` | 2025-08-09 | 上游 API13 安全節點；`cycleapple/ffxiv_visland@7150092` 完成 zh-TW 介面與 81 項 OC 英文／繁中工房製品對照，使用保存的 Dalamud API13 函式庫建置 0 警告、0 錯誤並發布 0.0.0.139 |
| Chat 2 | `Infiziert90/ChatTwo` | `956e57a` | 2025-11-17 | 上游 API13 安全節點；`cycleapple/ChatTwo@0b7b5ed` 更新有漏洞的依賴、完成資料庫／搜尋／字體與 Web 聊天介面 zh-TW，並補入台灣繁中字形、CJK 標點及全半形符號範圍；主插件以台服 API13 編譯成功（15 個既有警告、0 錯誤），發布 1.32.0.3 |
...
| MouseLock | `Zeffuro/MouseLock` | `fbabddc` / release `1.0.0.3` | 2026-08-08 | `cycleapple/MouseLock@953e9d1` 將 SDK 與目標框架移植至 Dalamud API13／.NET 9，補上 API13 服務與遊戲函式相容層並完成 zh-TW 介面；以台服 Dalamud 13.0.0.16 Debug／Release 建置 0 警告、0 錯誤，並以台服 `ffxiv_dx11.exe` 驗證 4 組簽名均唯一命中，發布 1.0.0.5 |
