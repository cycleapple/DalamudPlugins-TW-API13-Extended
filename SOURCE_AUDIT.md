...
| GatherBuddy Reborn | `AtmoOmen/GatherBuddyReborn` | `a34733dc` / tag `7.3.5.0` | 2025-12-02 | 位於安全日期範圍且 manifest 為 API13；`cycleapple/GatherBuddyReborn@5bc7d3f2` 保留台服繁中 fallback、`DataId`、AutoHook IPC 容錯、時間文字編碼與區域資料修正；參考 `aliceric27/DalamudPlugins-TW` 移植分級防卡死及定期指令，並修正升級措施未受近端失敗次數限制、區域中心不重設、Lifestream 僅固定等待，以及強制傳送後同幀仍繼續採集等缺口；API13 Release 建置 0 錯誤，發布 7.3.5.6 |
