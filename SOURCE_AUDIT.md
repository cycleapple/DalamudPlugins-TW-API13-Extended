...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` + `4liang0121/Ices-Cosmic-Exploration@21a4935` | `65a5806` | 2026-08-21 | 保留既有 ECommons、台服 API13、zh-TW 與 Artisan 同步修正；選擇性採用 fork 的逾時成果檢查、採集設定與任務繳交修復，並改以實際路線節點集合判斷有限採集枯竭、限制 Critical 繳交點的空間範圍、序列化設定保存及修正據點座標與掌握技能解鎖；排除 agenda、跨區移動、製作預設、共通巨集及整批路線資料；API13 Release 建置 0 錯誤，發布 0.0.73.50 |
