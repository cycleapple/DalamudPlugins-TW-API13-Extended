...
| Artisan | `MeowZWR/Artisan` | `e4a0ddaf` | 2025-12-13 | `cycleapple/Artisan@6825eee` 保留 `BaseId`、熱鍵列空參考與台服相容修正；修正素材清單庫存失效、依配方依賴順序重算半成品抵扣，並讓 Raphael 對宇宙探索動態配方正確套用與切換；API13 建置 0 錯誤，發布 4.0.4.22；上游已封存 |
...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` | `9d5a8f0` | 2026-08-03 | 保留 ECommons、`DataId` 與 `Player.JobId` 台服 API13 修正並完成一般使用者介面 zh-TW；依台服資料加入領地 1237 各探索階段 NPC ID 與座標備援；修正 ICE／Artisan 製作握手、重複送單、Raphael 運算逾時、修理流程卡住、未知確認視窗誤按及任務 ID 0 汙染；API13 建置 0 錯誤，發布 0.0.73.47 |
