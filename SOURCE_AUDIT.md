...
| AutoRetainer | `PunishXIV/AutoRetainer` | `1e4e6b6` / tag `4.5.1.12` | 2025-08-15 | API13 編譯成功；`cycleapple/AutoRetainer@9777264` 保留繁中公會工坊／組件功能比對，第三輪完成雇員委託規劃器 zh-TW，enum 僅透過 UI dictionary 顯示繁中；40 個既有警告、0 錯誤，發布 4.5.1.25 |
...
| AutoDuty | `ffxivcode/AutoDuty` | `a7d1eca8` | 2025-12-08 | `cycleapple/AutoDuty@a65c6a8` 保留既有路徑分頁容錯，第三輪補齊主頁路徑狀態、缺檔、依賴與執行驗證提示、練等模式、親信戰友、解除限制與職業限制 zh-TW；API13 建置 0 錯誤，發布 0.0.0.4 |
