...
| Artisan | `MeowZWR/Artisan` | `e4a0ddaf` | 2025-12-13 | `cycleapple/Artisan@b59a040` 保留 `BaseId`、熱鍵列空參考與台服相容修正；讓 Raphael 對宇宙探索動態配方正確套用，並完整管理求解中、成功、失敗、取消與逾時狀態；IPC 製作會等待有效解法且不暗中改用 Standard；API13 建置 0 錯誤，發布 4.0.4.23；上游已封存 |
...
| Ice's Cosmic Exploration | `cycleapple/Ices-Cosmic-Exploration` | `c80e715` | 2026-08-03 | 保留 ECommons、`DataId` 與 `Player.JobId` 台服 API13 修正並完成一般使用者介面 zh-TW；修正 ICE／Artisan 製作握手、重複送單與修理流程；整合 Raphael 配方狀態 IPC，統一一般與雙職製作入口，求解失敗時顯示原因並安全停止；API13 建置 0 錯誤，發布 0.0.73.48 |
