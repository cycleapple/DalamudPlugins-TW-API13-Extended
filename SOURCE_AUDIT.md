...
| Ice's Cosmic Exploration | `LeontopodiumNivale14/Ices-Cosmic-Exploration` | `f3b7a696c9` | 2025-12-15 | 保留 ECommons、`DataId` 與 `Player.JobId` 台服 API13 修正並完成一般使用者介面 zh-TW；另修正研究威上游 NPC ID 在台服找不到時的 null 例外，以目標或設定座標附近的可互動 NPC 安全解析；API13 建置 0 錯誤，發布 0.0.73.43 |
